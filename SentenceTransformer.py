...
class SentenceTransformer(nn.Sequential):
    ...
    def fit(self,
        ...
        with trange(epochs, desc="Epoch", disable=not show_progress_bar) as tepoch:
            # ------- tepoch scope -------
            for epoch in tepoch:
                training_steps = 0
                ...
                with trange(steps_per_epoch, desc="Iteration", smoothing=0.05,
                        disable=not show_progress_bar) as tstep:
                    # ------- tstep scope -------
                    for _ in tstep:
                        for train_idx in range(num_train_objectives):
                            ...
                            # Can be updated, we need only the loss value
                            historic_loss[i].append(loss_value.cpu().detach().numpy().tolist())
                        ...
                        loss_log_data = {'loss'+str(i): historic_loss[i][-1]
                                         for i in range(num_train_objectives)}

                        tstep.set_postfix(loss_log_data, refresh=False)
                ...
                loss_log_data = {'loss'+str(i): np.array(historic_loss[i]).mean()
                                 for i in range(num_train_objectives)}

                tepoch.set_postfix(loss_log_data, refresh=False)
                # ------- tepoch scope -------
        ...
        with open(os.path.join(output_path, 'loss_digest.json'), 'w') as fl:
            json.dump(historic_loss, fl)
